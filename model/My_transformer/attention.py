...
import sys
sys.path.append("/data/code/ETalk/model")
...
class Subspace_routing(nn.Module):
    def __init__(self,emo_size,feat_size,k,h_list) -> None: # h_list 表示每个子空间划分方法中head的数量
        ...
        for i in self.h_list:
            self.d_list.append(int(self.emo_size/i)) # 每个子空间的维度
        self.d_sum = int(sum(self.d_list)) # 所有子空间维度之和
        self.h_sum = int(sum(self.h_list)) # 所有子空间head数之和
        self.alpha = 1
    ...
    def subspace_routing(self,route_emo_feat, em_embedding, audio_embedding):
        B = route_emo_feat.shape[0] # batch size
        L_t = route_emo_feat.shape[1] # sequence length
        self.aggregation_k = L_t
        #M_Q
        M_Q_list = []
        for i in range(len(self.d_list)): # 对每个子空间划分方法，在特征维度上都构建mask
            ...
            if i==0:
                ...
                left = 0
                for j in range(i): # 计算每个子空间划分方法，在特征维度上对应的mask的起始位置
                    left += self.d_list[j]
        ...
        #M_V
        M_V_list = []
        for i in range(len(self.h_list)): # 对每个头构建mask
