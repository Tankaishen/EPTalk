...
def main():
    ...
    parser.add_argument("--pred_path", type=str, default="path/to/test")
    parser.add_argument("--gt_path", type=str, default="MEAD/vertices")
    parser.add_argument("--region_path", type=str, default="regions")
    parser.add_argument("--templates_path", type=str, default="MEAD/templates/template.npy")
    parser.add_argument("--model", type=str, default="")
    parser.add_argument("--num_sample", type=str)
    parser.add_argument("--dataset", type=str, default="MEAD")
    ...
    if args.dataset == "BIWI":
        ...
        nr_vertices = 5023
        sentence_list = ['sentence' + str(i).zfill(2) for i in range(1, 41)]

        # with open(args.templates_path, 'rb') as fin:
        #     templates = pickle.load(fin, encoding='latin1')
        templates = np.load(args.templates_path)
    ...
    mve = 0
    num_seq = 0
    for file in tqdm(os.listdir(args.pred_path)):
        if not file.endswith('.npy'):
            continue
        subject = file.split('_')[0]
        sentence = '_'.join(file.split('_')[1:])
        vertices_gt_path = os.path.join(args.gt_path, subject,sentence)
        vertices_pred_path = os.path.join(args.pred_path, file)
        if not os.path.exists(vertices_gt_path):
            continue
        if not os.path.exists(vertices_pred_path):
            continue
        vertices_gt = np.load(vertices_gt_path).reshape(-1, nr_vertices, 3)
        vertices_pred = np.load(vertices_pred_path).reshape(-1,nr_vertices,3)
        ...
        motion_pred = vertices_pred - templates.reshape(1, nr_vertices, 3)
        motion_gt = vertices_gt - templates.reshape(1, nr_vertices, 3)
