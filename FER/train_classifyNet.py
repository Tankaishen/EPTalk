...
sys.path.append('../')
sys.path.append('/data/code/EPTalk/')
from DEE.utils.utils import seed_everything, generate_date_time
from models.Classify import classifyNet
from datasets.affectnet_dataset import AffectNetExpressionDataset
...
def augment_expression_data(data, instance_prob=0.5, std=0.01, parameter_prob = 0.7):
    """
...
def train_one_epoch(model, data_loader, criterion, optimizer, device, epoch, writer, total_step,
    ...
    running_loss = 0.0
    template = np.load("MEAD/templates/template.npy").reshape(-1,15069)
    for i, (inputs, labels) in enumerate(data_loader):
        # inputs_ = inputs[:,:50]
        inputs_ = inputs - template
        inputs = augment_expression_data(inputs_,instance_prob=aug_prob, std=aug_std, parameter_prob = 0.7)
        ...
        if enable_wandb:
            ...
            writer.add_scalar(f'train/loss (step)', loss.item(), total_step + i)
        running_loss += loss.item()
    avg_loss = running_loss / len(data_loader)
    print(f'epoch [{epoch}] loss : {avg_loss:.5f}')
    if enable_wandb:
        # wandb.log({'train loss epoch' : avg_loss})
        writer.add_scalar(f'train/loss (epoch)', avg_loss, epoch)
    total_step+=total_steps
    return total_step
        
@torch.no_grad()
def val_one_epoch(model, data_loader, criterion, optimizer, device, epoch, writer, use_jaw=True, enable_wandb=True):
    model.eval()
    model.to(device)
    running_loss = 0.0
    total_steps = len(data_loader)
    ...
    if enable_wandb:
        ...
        writer.add_scalar(f'val/loss (epoch)', avg_loss, epoch)
        # wandb.log({'val acc epoch' : total_acc})
        writer.add_scalar(f'val/acc (epoch)', total_acc, epoch)
        for i, acc in enumerate(class_acc):
            # wandb.log({f'class acc/{i}':acc})
            writer.add_scalar(f'class/acc{i} (epoch)', acc, epoch)
    return avg_loss, total_acc
    
    
def main(args):
    cfg = get_yaml_config(args.config)
    log_path = 'FER' + str(time.strftime("%m_%d_%H_%M", time.localtime()))
    ...
    for epoch in range(cfg.train.num_epochs):
        total_step = train_one_epoch(model, train_loader, criterion, optimizer, device, epoch, 
                        writer=writer,
                        total_step=total_step,
                        use_jaw=use_jaw, 
                        aug_prob=cfg.train.data_aug.prob, 
                        aug_std=cfg.train.data_aug.std,
                        enable_wandb=args.wandb)
        scheduler.step()
        
        val_loss, accuracy = val_one_epoch(model, val_loader, criterion, optimizer, device, epoch, writer=writer, use_jaw=use_jaw, enable_wandb=args.wandb)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='FER/checkpoint/config_distribute.yaml', help='Path to the config file')
    parser.add_argument('--wandb', type=bool, default=True, help='Use wandb for logging')
    parser.add_argument('--device', type=str, default='cuda:1', help='Use wandb for logging')
