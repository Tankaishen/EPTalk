...
EMOTION_DICT = {'neutral': 0, 'happy': 1, 'sad': 2, 'angry' :  3, 'fear': 4, 'disgusted': 5, 'surprised': 6, 'contempt' : 7, 'calm': 8}
...
#                 'M022', 'M023', 'M024', 'M025', 'M026', 'M027', 'M028', 'M029'
#                 ] # 16 ids
# val_ids = ['M032', 'M033', 'M034', 'M035', 'W033']  # 5 ids

# test_ids = ['M037', 'M039', 'M040', 'M041', 'M042', 'W037'] # 8 ids
...
class AudioExpressionDataset(data.Dataset):
    def __init__(self, args, dataset='MEAD',split='train',vox_truncate=False):
        ...
        if args.use_30fps:
            self.fps = 30
        # data path
        self.audio_dir = os.path.join(args.audio_feature_dir)
        # self.audio_dir = os.path.join(args.audio_feature_dir ,self.dataset , 'audio_sample')
        self.expression_feature_dir = os.path.join(args.expression_feature_dir)
        ...
        if self.dataset == 'MEAD':
            ...
            for file_path in tqdm.tqdm(file_paths[:]):
                ...
                actor_name = uid[-2] # M005 # M005
                actor_id = MEAD_ACTOR_DICT[actor_name] # name -> id
                emotion = EMOTION_DICT[uid[-1].split('_')[0]]
                intensity = int(uid[-1].split("_")[2])
                gender = GENDER_DICT[actor_name[0]] # M -> 0, W -> 1
                
                param_dict = {}
                parameters = np.load(file_path)
                face = torch.tensor(parameters, dtype=torch.float32).reshape(parameters.shape[0],-1) #(len, 50)
                template = torch.tensor(np.load('/data/code/EPTalk/MEAD/templates/template.npy'),dtype=torch.float32).reshape(1,-1)
                expression_feature = face - template
                # try :
                ...
                #     expression_feature = torch.tensor(savgol_filter(expression_feature, 5, 2, axis=0))

                # audio_path = os.path.join(self.audio_dir, actor_name , uid + '.npy')
                audio_path = os.path.join(self.audio_dir, actor_name , uid[-1].replace("npy","wav"))
                if not os.path.exists(audio_path) :
                    print(f'{audio_path} doesnt exist')
                    continue
                audio_samples, _ = librosa.load(audio_path, sr=16000)
