...
from models.prob_models_clip import ProbDEE
import datasets
...
def train_one_epoch(args, model, optimizer,scheduler, train_dataloader, device, processor, total_step, writer, 
    ...
    for samples, labels in tqdm.tqdm(train_dataloader):
        ...
        audio_embedding, expression_embedding, audio_mean, exp_mean = model(audio_samples, expression_feat)
        ...
        loss = model.CLIP_loss(audio_mean,exp_mean, labels[0], device)
        total_loss += loss.item()
        if log_wandb:
            writer.add_scalar(f'train/loss(step)',loss.item() , total_step + step)
        if step % 50 == 0:
    ...
    scheduler.step()
    if log_wandb:
        writer.add_scalar(f'train/loss(epoch)', total_loss/total_steps, epoch)
...
def val_one_epoch(args, model, val_dataloader , device, processor, total_step, writer, affectnet_feature_extractor=None, epoch=None, log_wandb = True):
    ...
    for samples, labels_ in tqdm.tqdm(val_dataloader):
        ...
        loss = model.CLIP_loss(audio_mean,exp_mean, labels, device)
        total_loss += loss.item()
        if step % 50 == 0:
            print(f"epoch {epoch}, step {step} loss: {loss.item()}")
        ...
        if log_wandb:
            ...
            writer.add_scalar('val/loss(step)',loss , total_step + step)
        # writer.add_scalar('val/val loss', cumulative_loss/batch_num_per_epoch , epoch)
    writer.add_scalar('val/loss(epoch)', total_loss/totalsample , epoch)
