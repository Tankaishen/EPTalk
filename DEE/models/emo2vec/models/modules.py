...
class AltBlock(nn.Module):
    ...
    def forward_(self, x, padding_mask=None, alibi_bias=None):
    ...
    def forward(self, x, padding_mask=None, alibi_bias=None, output_attentions=False):
        if self.layer_norm_first:
            if output_attentions:
                a, attn_ = self.attn(self.norm1(x), padding_mask, alibi_bias, output_attentions=output_attentions)
            else:
                a = self.attn(self.norm1(x), padding_mask, alibi_bias)
...
class AltAttention(nn.Module):
    ...
    def forward(self, x, padding_mask=None, alibi_bias=None, output_attentions=False):
        ...
        attn_ = attn.softmax(dim=-1, dtype=torch.float32).to(dtype=dtype)
