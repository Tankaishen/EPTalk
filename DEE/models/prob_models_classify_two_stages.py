...
class ProbDEE(nn.Module):
    def __init__(self, args):
        ...
        # self.classficationHead = ClassificationHead(args)
        self.classficationHead = MLP(512,[256,128,64],8,dropout=0.1, batch_norm = True,activation='LeakyReLU')
        if args.loss == 'csd':
            if args.add_n_mog >= 1:
    ...
    def forward(self, audio, raw_exp):
        ...
        audio_logits = self.classficationHead(audio_mean)
        exp_logits = self.classficationHead(exp_mean)
        return audio_logits, exp_logits
...
class ClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.out_proj = nn.Linear(config.feature_dim, 8)
