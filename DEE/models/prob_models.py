...
class ProbAudioEncoder(nn.Module): 
    ...
    def forward_base(self, audio) :
        ...
        std_feats = feats
        mean_feats = feats
        return feats, std_feats, mean_feats
    ...
    def forward(self, audio):
        feats, std_feats, mean_feats = self.forward_base(audio)
...
class ProbDEE(nn.Module):
    ...
    def forward(self, audio, raw_exp):
        audio_feats, audio_mean, audio_logvar = self.audio_encoder(audio)
        audio_emb = {}
        audio_emb['feats'] = audio_feats
        audio_emb['mean'] = audio_mean
        audio_emb['std'] = audio_logvar # actually log var but the name is kept for consistency
        
        exp_feats, exp_mean, exp_logvar = self.exp_encoder(raw_exp)
        exp_emb = {}
        exp_emb['feats'] = exp_feats
