    ...
    def __init__(self, args) :
        ...
        if not self.input_embeddings: # if input audio
            ...
            if args.use_finetuned_emo2vec:
                ...
                finetuned_model = AutoModel(model='/root/.cache/modelscope/hub/models/iic/emotion2vec_base_finetuned',
...
class ProbDEE(nn.Module):
    ...
    def forward(self, audio, raw_exp):
        audio_feats, audio_mean = self.audio_encoder(audio)        
        exp_feats, exp_mean = self.exp_encoder(raw_exp)
        audio_feats = l2norm(audio_feats, dim=-1)
        exp_feats = l2norm(exp_feats, dim=-1)
        return audio_feats, exp_feats, audio_mean, exp_mean

    def CLIP_loss(self, audio_embedding, expression_embedding, ori_labels, device):
        labels = create_label_matrix(ori_labels).to(audio_embedding.device)
        # add clipping
        logits = expression_embedding @ audio_embedding.T * torch.clip(self.temperature.exp(), max = 100) # (batch_size, batch_size)
