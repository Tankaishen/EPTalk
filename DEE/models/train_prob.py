...
sys.path.append("/share/home/202321044040/data/code/DEEPTalk/DEE")
import torch
os.environ["WANDB__SERVICE_WAIT"] = "300"
import argparse
from config import get_args_parser
import models
import DEE.datasets as datasets
...
def train_one_epoch(args, model, optimizer,scheduler, train_dataloader, device, processor, writer, total_step, epoch=None,log_wandb = True):
    cumulative_loss = 0
    step = 0
    ...
    for samples, labels in tqdm.tqdm(train_dataloader):
        ...
        if log_wandb:
            ...
            writer.add_scalar("train/learning_rate",optimizer.param_groups[0]['lr'],total_step+step)
...
@torch.no_grad()
def val_one_epoch(args, model, val_dataloader , device, processor, writer, epoch=None, log_wandb = True):
    cumulative_loss = 0
    step = 0
    model.eval()
    criterion = model.criterion
    ...
    if log_wandb:
        ...
        writer.add_scalar("val/expression accuracy loss",exp_retrieve_accuracy, epoch)
        writer.add_scalar("val/audio accuracy loss",audio_retrieve_accuracy, epoch)
...
def main(args): 
    ...
    DEE = DEE.to(device)
    processor = Wav2Vec2Processor.from_pretrained("/share/home/202321044040/data/code/wav2vec2-base-960h")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        total_step = train_one_epoch(args, DEE, optimizer,scheduler, train_dataloader, device,processor, writer, total_step, epoch=epoch, log_wandb = True)
        print('training time for this epoch :', time.time() - training_time)
        
        validation_time = time.time()
        val_one_epoch(args, DEE, val_dataloader, device,processor, writer, epoch=epoch, log_wandb = True)
...
if __name__ == '__main__':
    ...
    log_path = "ProbDEE_" + str(time.strftime("%m_%d_%H_%M", time.localtime()))
